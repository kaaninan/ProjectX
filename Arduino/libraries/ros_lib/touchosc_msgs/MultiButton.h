#ifndef _ROS_touchosc_msgs_MultiButton_h
#define _ROS_touchosc_msgs_MultiButton_h
...
namespace touchosc_msgs
{
  ...
  class MultiButton : public ros::Msg
  {
    public:
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 0) = (this->dimension[i] >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->dimension[i] >> (8 * 1)) & 0xFF;
      offset += sizeof(this->dimension[i]);
      }
      for( uint8_t i = 0; i < 2; i++){
      union {
        float real;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      for( uint8_t i = 0; i < 2; i++){
      this->dimension[i] =  ((uint16_t) (*(inbuffer + offset)));
      this->dimension[i] |= ((uint16_t) (*(inbuffer + offset + 1))) << (8 * 1);
      offset += sizeof(this->dimension[i]);
      }
      for( uint8_t i = 0; i < 2; i++){
      union {
    ...
    const char * getType(){ return "touchosc_msgs/MultiButton"; };
    const char * getMD5(){ return "880b03eb2c058ce5a9ec27c84df03870"; };
