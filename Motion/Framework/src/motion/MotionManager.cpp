    ...
    if (m_CalibrationStatus == 0 || m_CalibrationStatus == -1)
        {
            ...
            if (buf_idx < GYRO_WINDOW_SIZE)
                {
                    m_ArbotixPro->ReadWord(0, ArbotixPro::P_GYRO_Y_L, &value, &error);
                    fb_gyro_array[buf_idx] = value;

                    m_ArbotixPro->ReadWord(0, ArbotixPro::P_GYRO_X_L, &value, &error);
                    rl_gyro_array[buf_idx] = value;
